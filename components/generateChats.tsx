...
function GenerateChats() {
    ...
    useEffect(() => {
        function getGenerates() {
            ...
            setGeneratesState(chats?.length > 0 ? [...chats]?.reverse() : [])
        }
    ...
    return chatsState?.map(Chat => {
        ...
        return (
            <div
                ...
                key={id}
                className={`${isArticleActive && 'bg-[#6C757D] border-[1px] border-[#6C757D]'} cursor-pointer group relative flex flex-col p-2 rounded-[10px] bg-[#343A40] border-[1px] border-[#495057] min-h-[90px] text-white`}
            >
                <div className=' flex flex-row justify-between items-center relative w-full'>
                    <h1 className='font-semibold relative max-h-[50px] w-[95%] overflow-y-auto break-words'>{prompt}</h1>
                    <svg
                        onClick={(e) => {
                            e.stopPropagation()
                ...
                <p className='font-light line-clamp-2'>{completion}</p>
            </div>
        )
    })
}
